...
private:
	...
	cv::Point _currTL;
public:
	BoundedPhysicsDecorator(IPhysicsComponentPtr base):_base(base){
		int screenWidth = GetSystemMetrics(SM_CXFULLSCREEN);
		int screenHeight = GetSystemMetrics(SM_CYFULLSCREEN);
		_bounds = cv::Rect(0, 0, screenWidth, screenHeight);
	}
	void test();
	// TODO:
	virtual void reset(cv::Point const& tl);
...
public:
	...
	JumpPhysics(int horizontalVelocity, int initialVerticalVelocity, int gravity,int finishHight);
	virtual void reset(cv::Point const& tl);
	virtual bool update(cv::Mat const& collisionMask);
	virtual cv::Mat const& getCollisionMask() const override;
	virtual bool checkCollision(IPhysicsComponentPtr const& other) const;
