...
void Entity::onNotify(Event const& e)
{
		...
		e.code == EventCodes::COLLISION_WITH_ENEMY && newStateCandidate)
	{
		...
		if (_state->getPhysics()->getVelocity().x < 0)
			newStateCandidate->reset(cv::Point(pos_x + 50, pos_y));
		else
			_state->reset(cv::Point(pos_x - 50, pos_y));
...
bool Entity::checkCollision(std::shared_ptr<Entity> other)
{
	if (this->_state->getPhysics()->checkCollision(other->_state->getPhysics())) {
		...
		this->Notify(Event{ EventSenders::SENDER_ENTITY_STATE, EventTypes::EVENT_PHYSICS, EventCodes::COLLISION_WITH_ENEMY });
		return true;
	}
	return false;
}
